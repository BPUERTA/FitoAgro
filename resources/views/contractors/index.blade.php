<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-start justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Contratistas</h1>
                <p class="text-sm text-gray-500">Gestión de contratistas registrados.</p>
            </div>
        ...
        <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200">
                ...
                <span class="text-xs text-gray-500">Total: {{ $contractors->count() }}</span>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nombre / Razón Social</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">CUIT</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Equipos</th>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse($contractors as $contractor)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-sm text-gray-700">{{ $contractor->numero }}</td>

                                <td class="px-4 py-3 text-sm font-medium text-gray-900">
                                    {{ $contractor->nombre }}
                                </td>

                                <td class="px-4 py-3 text-sm text-gray-900">{{ $contractor->cuit }}</td>

                                <td class="px-4 py-3 text-sm text-gray-900">
                                    <span class="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">
                                        {{ $contractor->teams->count() }} equipos
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-3 text-right whitespace-nowrap">
                                    ...
                                    @if(auth()->user()->is_admin || auth()->user()->is_org_admin)
                                        ...
                                        <form action="{{ route('contractors.destroy', $contractor) }}" method="POST" class="inline">
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                    onclick="return confirm('¿Eliminar contratista?')"
