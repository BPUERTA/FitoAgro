        ...
        <form action="{{ route('registro-tecnicos.update', $registroTecnico) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl p-6 space-y-4">
                ...
                <div>
                    ...
                    <select name="client_selector" id="client_selector" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500">
                        ...
                        <optgroup label="Clientes">
                            @foreach($clients as $client)
                                ...
                                <option value="{{ $clientValue }}" {{ old('client_selector') === $clientValue || (!old('client_selector') && old('client_id', $registroTecnico->client_id) == $client->id) ? 'selected' : '' }}>
                    ...
                    <input type="hidden" name="client_id" id="client_id" value="{{ old('client_id', $registroTecnico->client_id) }}">
                </div>
                ...
                <div>
                    ...
                    <select name="farm_id" id="farm_id" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500">
                        ...
                        @foreach($farms as $farm)
                            <option value="{{ $farm->id }}" data-client="{{ $farm->client_id }}" data-group="{{ $farm->client_group_id }}" {{ old('farm_id', $registroTecnico->farm_id) == $farm->id ? 'selected' : '' }}>
                                {{ $farm->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="lot_id" id="lot_id" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500">
                        ...
                        @foreach($farms as $farm)
                            @foreach($farm->lots as $lot)
                                <option value="{{ $lot->id }}" data-client="{{ $farm->client_id }}" data-group="{{ $farm->client_group_id }}" data-farm="{{ $farm->id }}" {{ old('lot_id', $registroTecnico->lot_id) == $lot->id ? 'selected' : '' }}>
                                    {{ $farm->name }} - {{ $lot->name }}
                                </option>
                            @endforeach
                        @endforeach
