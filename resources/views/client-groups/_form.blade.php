                ...
                <div class="flex flex-col md:flex-row md:flex-nowrap gap-3 items-stretch md:items-center member-row">
                    ...
                    <div class="flex items-center gap-2 md:w-64">
                        ...
                        <label class="flex items-center gap-1 text-xs text-gray-600 whitespace-nowrap">
                            <input type="checkbox" class="fixed-toggle rounded border-gray-300 text-green-600 focus:ring-green-500" {{ isset($member['percentage']) && $member['percentage'] !== '' ? 'checked' : '' }}>
                            Fijar
                        </label>
        ...
        function distributePercentages() {
            ...
            rows.forEach((row) => {
                ...
                if (fixed?.checked) {
                    ...
                    unfixedRows.push({ row, input });
                }
            });

            const remaining = Math.max(0, 100 - fixedTotal);
            const share = unfixedRows.length ? remaining / unfixedRows.length : 0;

            unfixedRows.forEach(({ input }) => {
                if (!input) return;
                input.value = share.toFixed(2).replace(/\.00$/, '');
            });
        }
