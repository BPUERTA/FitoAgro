<x-app-layout>
    <div class="w-full px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">Editar Grupo de Clientes</h1>
            <p class="text-sm text-gray-500">{{ $clientGroup->name }}</p>
        </div>
        ...
        <form action="{{ route('client-groups.update', $clientGroup) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            @include('client-groups._form', ['clients' => $clients, 'clientGroup' => $clientGroup])
