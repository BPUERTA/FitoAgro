<x-app-layout>
    <div class="w-full px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">Edit Work Order</h1>
            <p class="text-sm text-gray-500">Update work order {{ $workOrder->code ?? $workOrder->id }}</p>
        </div>
        ...
        <form action="{{ route('work-orders.update', $workOrder) }}" method="POST" id="workOrderForm">
            @csrf
            @method('PUT')
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <select name="priority" id="priority"
                                    ...
                                    @foreach($priorities as $key => $label)
                                        <option value="{{ $key }}" {{ old('priority', $workOrder->priority) == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="scheduled_start_at" id="scheduled_start_at"
                                       value="{{ old('scheduled_start_at', $workOrder->scheduled_start_at?->format('Y-m-d\\TH:i')) }}"
                                       class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="scheduled_end_at" id="scheduled_end_at"
                                       value="{{ old('scheduled_end_at', $workOrder->scheduled_end_at?->format('Y-m-d\\TH:i')) }}"
                                       class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                        class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">
                                    <option value="pendiente" {{ old('status', $workOrder->status) == 'pendiente' ? 'selected' : '' }}>Pending</option>
                                    <option value="abierto" {{ old('status', $workOrder->status) == 'abierto' ? 'selected' : '' }}>Open</option>
                                    <option value="cerrado" {{ old('status', $workOrder->status) == 'cerrado' ? 'selected' : '' }}>Closed</option>
                                    <option value="cancelado" {{ old('status', $workOrder->status) == 'cancelado' ? 'selected' : '' }}>Canceled</option>
                                </select>
                            </div>
                            ...
                            <div id="canceledReasonWrapper" class="hidden">
                                ...
                                <textarea name="canceled_reason" id="canceled_reason" rows="2"
                                          class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">{{ old('canceled_reason', $workOrder->canceled_reason) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="3"
                                          class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">{{ old('description', $workOrder->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const existingFarms = @json($existingFarms ?? []);
            ...
            function populateFarmSelectOptions(selectEl) {
                const selectedValue = selectEl.dataset.selectedValue || selectEl.value;
            ...
            function createFarmRow(prefill = {}) {
                ...
                if (prefill.exploitation_id) select.dataset.selectedValue = prefill.exploitation_id;
                ...
                select.addEventListener('change', function() {
                    ...
                    select.dataset.selectedValue = this.value;
                });
            ...
            existingFarms.forEach(farm => blocksSet.add(farm.bloque || 'A'));
            existingProducts.forEach(product => blocksSet.add(product.bloque || 'A'));
            if (blocksSet.size === 0) blocksSet.add('A');

            Array.from(blocksSet).sort().forEach(letter => {
                blockMap[letter] = addBlock(letter);
            });

            existingFarms.forEach(farm => {
                const letter = farm.bloque || 'A';
                const blockEl = blockMap[letter] || addBlock(letter);
                blockMap[letter] = blockEl;
                blockEl.querySelector('.farm-rows').appendChild(createFarmRow(farm));
            });

            existingProducts.forEach(product => {
                const letter = product.bloque || 'A';
                const blockEl = blockMap[letter] || addBlock(letter);
                blockMap[letter] = blockEl;
                blockEl.querySelector('.product-rows').appendChild(createProductRow(product));
            });
